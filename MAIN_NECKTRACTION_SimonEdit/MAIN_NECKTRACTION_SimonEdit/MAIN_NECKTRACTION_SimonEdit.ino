...
double Kp = 2.5; // Proportional Gain (TBD)
double Kd = 1; // Derivative Gain (TBD)
double Ki = 0.1; // Integral Gain (TBD)
...
void loop() {
  ...
  switch (CaseValMain) {
    ...
    case 2: // Additional Cases can be added to add progressive checks to stop motors and ask for prompts to continue
      ...
      if ((abs(DiffDesired) <= 0.05 * desiredForce)) {
  ...
  if (myPID.GetMode() == AUTOMATIC) {
    ...
    travelSpeedPID = constrain(travelSpeedPID, -25, 25); // constrain travel speed to be within reasonable value
